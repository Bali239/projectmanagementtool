...
export default function TaskCard({ task }: { task: BoardTask }) {
  ...
  return (
    <article ref={ref} className={`rounded-md border border-slate-200 bg-white p-3 shadow-[0_1px_2px_rgba(15,23,42,0.04)] transition-shadow hover:shadow-md ${isDragging ? "opacity-50" : ""}`}>
      ...
      <div className="flex min-h-6 flex-wrap items-center justify-between gap-2">
        <Tag bordered={false} className="m-0 bg-slate-100 font-mono text-[10px] text-slate-500">{task.id.slice(0, 8)}</Tag>
