          ...
          {openMenu === "account" && (
            <div className="absolute right-0 top-11 w-60 rounded-xl border border-slate-200 bg-white p-1.5 shadow-xl shadow-slate-900/10">
              <div className="border-b border-slate-100 px-3 py-2.5">
                ...
                <p className="mt-0.5 truncate text-xs text-slate-500">{user?.email || "Workspace member"}</p>
              </div>
              <button type="button" disabled={signOutMutation.isPending} onClick={() => signOutMutation.mutate()} className="flex w-full items-center gap-2.5 rounded-lg px-2.5 py-2 text-left text-sm text-slate-700 hover:bg-slate-50 disabled:opacity-50"><LogOut className="size-4 text-slate-400" /> {signOutMutation.isPending ? "Signing out..." : "Sign out"}</button>
            </div>
          )}
        </div>
      </div>
